...
namespace CherryMP.Util
{
    public static class NativeWhitelist
    {
        public static void Init()
        {
            ...
            using (var file = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("CherryMP.natives.txt")))
            {
                string currentLine;
    ...
    public static class SoundWhitelist
    {
        public static void Init()
        {
            ...
            using (var file = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("CherryMP.soundlist.txt")))
            {
                string currentLine;
                while (!string.IsNullOrEmpty((currentLine = file.ReadLine())))
                {
