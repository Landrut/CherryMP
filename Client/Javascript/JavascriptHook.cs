...
using Vector3 = CherryMPShared.Vector3;
using VehicleHash = CherryMPShared.VehicleHash;
using WeaponHash = CherryMPShared.WeaponHash;
using System.ComponentModel;

namespace CherryMP.Javascript
{
    ...
    public class ScriptContext
    {
        ...
        */
        ...
        public void setPlayerFacialIdleAnimOverride(LocalHandle player, string animName)
        {
            Function.Call(Hash.SET_FACIAL_IDLE_ANIM_OVERRIDE, player.Value, "facials@gen_female@base", animName);
        }

        public void clearPlayerFacialIdleAnimOverride(LocalHandle player)
        {
            Function.Call(Hash.CLEAR_FACIAL_IDLE_ANIM_OVERRIDE, player.Value);
        }

        public void setPlayerMovementClipset(LocalHandle player, string animSet, double speed)
        {
            Function.Call(Hash.SET_PED_MOVEMENT_CLIPSET, player.Value, animSet, speed);
        }

        public void resetPlayerMovementClipset(LocalHandle player)
        {
            Function.Call(Hash.RESET_PED_MOVEMENT_CLIPSET, player.Value, 0.0f);
        }

        public double getAnimCurrentTime(LocalHandle player, string animDict, string animName)
        {
            double AnimCurrentTime = Function.Call<double>(Hash.GET_ENTITY_ANIM_CURRENT_TIME, Game.Player.Character, animDict, animName);
            return AnimCurrentTime;
        }
        ...
        public void setPlayerHeadBlendData(LocalHandle player, int shapeFirstId, int shapeSecondId, int shapeThirdId, int skinFirstId, int skinSecondId, int skinThirdId, double shapeMix, double skinMix, double thirdMix, bool isParent)
        {
            Function.Call(Hash.SET_PED_HEAD_BLEND_DATA, player.Value, shapeFirstId, shapeSecondId, shapeThirdId, skinFirstId, skinSecondId, skinThirdId, shapeMix, skinMix, thirdMix, isParent);
        }

        /*public void setPlayerHeadBlendData(LocalHandle player, int shapeFirstId, int shapeSecondId, int shapeThirdId, int skinFirstId, int skinSecondId, int skinThirdId, float shapeMix, float skinMix, float thirdMix, bool isParent)
        {
            Function.Call(Hash.SET_PED_HEAD_BLEND_DATA, player.Value, shapeFirstId, shapeSecondId, shapeThirdId, skinFirstId, skinSecondId, skinThirdId, shapeMix, skinMix, thirdMix, isParent);
        }*/

        public void updatePlayerHeadBlendData(LocalHandle player, double shapeMix, double skinMix, double thirdMix)
        {
            Function.Call(Hash.UPDATE_PED_HEAD_BLEND_DATA, player.Value, shapeMix, skinMix, thirdMix);
        }

        public bool hasPlayerHeadBlendFinished(LocalHandle player)
        {
            return Function.Call<bool>(Hash.HAS_PED_HEAD_BLEND_FINISHED, player.Value);
        }

        public void setPlayerFaceFeature(LocalHandle player, int index, double scale)
        {
            Function.Call(Hash._SET_PED_FACE_FEATURE, player.Value, index, scale);
        }

        public void setPlayerHeadOverlay(LocalHandle player, int overlayId, int partIndex, double opacity)
        {
            Function.Call(Hash.SET_PED_HEAD_OVERLAY, player.Value, overlayId, partIndex, opacity);
        }

        public void setPlayerHeadOverlayColor(LocalHandle player, int overlayId, int colorType, int colorId, int secondColorId)
        {
            Function.Call(Hash._SET_PED_HEAD_OVERLAY_COLOR, player.Value, overlayId, colorType, colorId, secondColorId);
        }

        public void setPlayerHeadOverlayColor(LocalHandle player, int overlayId, int colorType, int colorId, double opacity)
        {
            Function.Call(Hash._SET_PED_HEAD_OVERLAY_COLOR, player.Value, overlayId, colorType, colorId, opacity);
        }

        public void setPlayerEyeColor(LocalHandle player, int index)
        {
            Function.Call(Hash._SET_PED_EYE_COLOR, player.Value, index);
        }

        public void setPlayerHairColor(LocalHandle player, int hairColorId, int hairHighlightId)
        {
            Function.Call(Hash._SET_PED_HAIR_COLOR, player.Value, hairColorId, hairHighlightId);
        }

        /*public void setPlayerHairStyle(LocalHandle player, int id, int colorId, int highlightId, string collection, string overlay)
        {
            Function.Call(Hash.SET_PED_COMPONENT_VARIATION, player.Value, 2, wt, colorId, highlightId);
        }*/

        public void clearPlayerFacialDecorations(LocalHandle player)
        {
            Function.Call(Hash._CLEAR_PED_FACIAL_DECORATIONS, player.Value);
        }

        public int getNumMakeupColors()
        {
            return Function.Call<int>(Hash._GET_NUM_MAKEUP_COLORS);
        }
        ...
        public int getNumHeadOverlayValues(int overlayId)
        {
            return Function.Call<int>(Hash._GET_NUM_HEAD_OVERLAY_VALUES, overlayId);
        }
...
#region Menus
        ...
        public UIMenu addSubMenu(UIMenu menu, string text, string description, double x, double y, int anchor, bool enableBanner = true, bool disableControls = false, bool scaleWithSafezone = false)
        {
            var offset = convertAnchorPos((float)x - 10, (float)y - 107, (Anchor)anchor, 431f, 107f + 38 + 38f * 10);
            var item = new UIMenuItem(text, description);
            menu.AddItem(item);
            var submenu = new UIMenu(menu.Title.Caption, text, new Point((int)offset.X, (int)offset.Y))
            {
        ...
        public void enableInteriorProp(int interiorId, string propName)
        {
            Function.Call(Hash._ENABLE_INTERIOR_PROP, interiorId, propName);
        }
