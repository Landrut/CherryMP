    ...
    public class TabMissionSelectItem : TabItem
    {
        ...
        public override void Draw()
        {
            ...
            for (int i = _minItem; i < Math.Min(Heists.Count, _maxItem); i++)
            {
                ...
                new UIResText(Heists[i].Name, SafeSize.AddPoints(new Point(6, 5 + (itemSize.Height + 3) * counter)), 0.40f, Color.FromArgb(fullAlpha, (Index == i && Focused) ? Color.Black : Color.White)).Draw();
                counter++;
            }
            ...
            for (int i = 0; i < Heists[Index].ValueList.Count; i++)
            {
                ...
                new UIResText(label, new Point((int)res.Width - SafeSize.X - 6, SafeSize.Y + 260 + 42 + (40 * i)), 0.45f, Color.FromArgb(fullAlpha, Color.White), Font.ChaletComprimeCologne, UIResText.Alignment.Right).Draw();
            }
