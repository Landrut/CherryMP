    ...
    public class TabInteractiveListItem : TabItem
    {
        ...
        public override void Draw()
        {
            ...
            for (int c = _minItem; c < Math.Min(Items.Count, _maxItem); c++)
            {
                ...
                new UIResText(Items[c].Text, SafeSize.AddPoints(new Point((hasBothBadges ? 60 : hasAnyBadge ? 30 : 6), 5 + (itemSize.Height + 3) * i)), 0.45f, Color.FromArgb(fullAlpha, (Index == c && Focused) ? Color.Black : Color.White)).Draw();
                ...
                if (!string.IsNullOrEmpty(Items[c].RightLabel))
                {
                    new UIResText(Items[c].RightLabel,
                        ...
                        0.45f, Color.FromArgb(fullAlpha, (Index == c && Focused) ? Color.Black : Color.White),
                        Font.ChaletComprimeCologne, UIResText.Alignment.Right).Draw();
                }
