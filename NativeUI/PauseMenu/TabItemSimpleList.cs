    ...
    public class TabItemSimpleList : TabItem
    {
        ...
        public override void Draw()
        {
            ...
            for (int i = 0; i < Dictionary.Count; i++)
            {
                ...
                new UIResText(item.Key, new Point(TopLeft.X + 6, TopLeft.Y + 5 + (40 * i)), 0.40f, Color.FromArgb(fullAlpha, Color.White)).Draw();
                new UIResText(item.Value, new Point(BottomRight.X - 6, TopLeft.Y + 5 + (40 * i)), 0.40f, Color.FromArgb(fullAlpha, Color.White), Font.ChaletComprimeCologne, UIResText.Alignment.Right).Draw();
            }
        }
    }
}